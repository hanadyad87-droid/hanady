<template>
  <div class="flex flex-col gap-6" dir="rtl">
    ...
    <!-- عنوان الصفحة -->
    <h2 class="text-2xl font-bold text-blue-800">طلب نقل قسم</h2>
    ...
    <div class="bg-white shadow rounded-xl p-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-gray-700">
        ...
        <p><span class="font-semibold">الإدارة الحالية:</span> {{ employee?.Department || '-' }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white shadow rounded-xl p-4 flex flex-col gap-4">
        <h3 class="font-bold text-lg mb-2">إرسال طلب النقل</h3>

        <select v-model="newDepartment" class="border rounded-lg p-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 w-full sm:w-3/4">
          <option disabled value="">اختر القسم الجديد</option>
          <option v-for="dept in departments" :key="dept" :value="dept">{{ dept }}</option>
        </select>

        <textarea v-model="note" placeholder="سبب النقل" class="border rounded-lg p-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none w-full sm:w-3/4"></textarea>
      ...
      <div class="bg-white shadow rounded-xl p-4 overflow-x-auto">
        <h3 class="font-bold text-lg mb-4">طلبات النقل السابقة</h3>
        ...
        <table class="min-w-full divide-y divide-gray-200 text-right">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-4 py-2 text-sm font-medium text-gray-700">القسم الجديد</th>
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="req in requests" :key="req.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-2 text-sm text-gray-700">{{ req.newDepartment }}</td>
...
export default {
  ...
  data() {
    return {
      newDepartment: "",
      note: "",
      departments: ["الموارد البشرية", "المحاسبة", "التسويق", "تكنولوجيا المعلومات", "الشؤون الإدارية"],
      requests: [
        { id: 1, date: "2026-01-05", newDepartment: "الموارد البشرية", status: "مقبول" },
        { id: 2, date: "2026-01-06", newDepartment: "التسويق", status: "تحت المراجعة" }
      ],
      toastMessage: "",
      toastType: "success"
    };
  ...
  if (!this.newDepartment) {
    this.toastMessage = "الرجاء اختيار القسم الجديد";
  ...
  // عرض الاشعار بالنجاح
  this.toastMessage = "تم إرسال طلب النقل ✅";
  this.toastType = "success";

  // اجعل الاشعار يختفي بعد ثانيتين
  setTimeout(() => {
    this.toastMessage = "";
  }, 2000);
}
