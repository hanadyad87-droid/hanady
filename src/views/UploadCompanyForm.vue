<template>
  <div class="flex min-h-screen bg-gray-100 font-cairo" dir="rtl">
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6 mt-4">
        ...
        <div class="mb-4">
          <input
            ...
            class="input w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary outline-none text-right"
          />
        </div>
          ...
          <table class="min-w-full text-right divide-y divide-gray-200">
            <thead class="bg-navbar">
              <tr>
                ...
                <th class="p-3 text-sm font-semibold text-gray-600">تاريخ الرفع</th>
                <th class="p-3 text-sm font-semibold text-gray-600">الإجراءات</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="formItem in filteredForms" :key="formItem.id" class="hover:bg-gray-50 transition">
                <td class="p-3 text-sm font-medium text-gray-700">{{ formItem.title }}</td>
                <td class="p-3 text-sm text-gray-600">{{ formItem.uploadedBy }}</td>
                <td class="p-3 text-sm text-gray-500">{{ formatDate(formItem.uploadedAt) }}</td>
                <td class="p-3 text-sm flex gap-3">
                  <!-- View Description -->
                  <button @click="viewForm(formItem)" class="text-blue-600 hover:scale-110 transition" title="عرض الوصف">
    ...
    <!-- Modal رفع نموذج (موحد مع ستايل إضافة مؤهل) -->
    <div v-if="showUploadModal" class="fixed inset-0 bg-black/50 flex justify-center items-center z-[60] p-4">
      <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <input type="file" @change="onFileChange" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-primary file:text-white cursor-pointer"/>
          </div>
        </div>
...
export default {
  ...
  setup() {
    ...
    // البحث الموحد (يعمل حسب العنوان أو اسم من رفعه)
    ...
    const viewForm = (formData) => {
      selectedDescription.value = formData.description || "لا يوجد وصف متوفر لهذا النموذج.";
    ...
    const uploadForm = async () => {
      ...
      try {
        ...
        toast("فشل في رفع النموذج، حاول مرة أخرى", "error");
      }
    }
    ...
    return {
      ...
      selectedDescription, filteredForms, confirmDeleteId,
